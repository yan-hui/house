...
/**
 * @Description httpclient默认配置
 * @Author hechunhui
 * @CreatedBy 2018/7/27 10:46
 */
@Configuration
...
public class HttpClientAutoConfiguration {
    ...
    public HttpClient httpClient() {
        RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(properties.getConnectTimeOut())
                .setSocketTimeout(properties.getScoketTimeOut()).build();//构建requestonfig
        HttpClient client = HttpClientBuilder.create().setDefaultRequestConfig(requestConfig).setUserAgent(properties.getAgent())
                ...
                .setConnectionReuseStrategy(new NoConnectionReuseStrategy()).build();//设置不进行重试
        return client;
    }
